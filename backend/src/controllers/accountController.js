...
export const deposit = async (req, res) => {
  try {
    ...
    if (account.userId.toString() !== req.user.userId) {
      return res
        .status(403)
        .json({ error: "Forbidden: You do not own this account." });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to process deposit: " + err.message });
  }
};
...
export const withdraw = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to process withdrawal: " + err.message });
  }
};
...
export const transfer = async (req, res) => {
  ...
  try {
    ...
    if (fromAccount.userId.toString() !== req.user.userId) {
      ...
      return res
        .status(403)
        .json({ error: "Forbidden: You do not own the source account." });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to process transfer: " + err.message });
  }
};
