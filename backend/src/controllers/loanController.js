...
export const getLoanDetails = async (req, res) => {
  try {
    ...
    if (!loan) {
      return res
        .status(404)
        .json({ message: "Loan not found or you do not have access." });
    }
    res.json(loan);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
export const payLoanInstallment = async (req, res) => {
  ...
  try {
    ...
    const repayment = await Repayment.findOne({ loanId: loan._id, paid: false })
      .sort({ dueDate: 1 })
      .session(session);
    if (!repayment) {
      await session.abortTransaction();
      ...
      return res
        .status(400)
        .json({ message: "No outstanding payments for this loan." });
    }
    ...
    if (!account || account.balance < repayment.amount) {
      ...
      return res
        .status(400)
        .json({ message: "Insufficient balance to make this payment." });
    }
